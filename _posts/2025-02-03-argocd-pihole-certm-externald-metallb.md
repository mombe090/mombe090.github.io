...
*Le Gestionnaire de Paquets Kubernetes*
...
*Outils de configuration très populaire sur kuberenetes*
...
![Argo](https://github.com/argoproj/argo-cd/blob/b4a63aeba87d5d4d7b0c72a608fe873abc8be7f6/docs/assets/argo.png?raw=true)
*L'outils GitOps le plus populaire pour Kubernetes*
  ...
  *C'est pas pour rien qu'il est l'outil `GitOps` qui a le plus `d'étoiles` sur github.*
...
![Metallb Blue](https://github.com/metallb/metallb/blob/main/website/static/images/logo/metallb-blue.png?raw=true)
*MetallB projet actuellement en phase de sandbox à la `CNCF`*
...
![Logo Small](https://github.com/cert-manager/cert-manager/blob/master/logo/logo-small.png?raw=true)
*Cert-Manager le gestionnaire de certificats TLS/SSL par excélence pour `kubernetes`*
...
![Contour](https://raw.githubusercontent.com/projectcontour/contour/main/contour.png)
*Contour est un Ingress Controller pour kubernetes actuellement en incubation à la CNCF*
...
![Vortex Vertical Wordmark Darkmode](https://github.com/pi-hole/graphics/raw/master/Vortex/Vortex_Vertical_wordmark_darkmode.png?raw=true)
*Le bloqueur de publicité et serveur DNS aimé par les homelab-(eurs/euses).*
...
![External Dns](https://github.com/kubernetes-sigs/external-dns/blob/master/docs/img/external-dns.png?raw=true)
*external-dns pour kubernetes*
...
- Ou utiliser la commande argocd pour forcer la synchronisation
  ...
  - Sur le dashboard d'ArgoCd, on peut voir que les applications sont synchronisées avec le cluster k8s.
    - ![Argo Sync Sample](../assets/img/content/argo-sync-sample.webp)
  ...
  - Une fois la synchronisation terminée, vous devriez avoir les 2 nouveaux objets dans l'application `infra-app` :
    ![Argo Cd Sync Metallb Crd](../assets/img/content/argo-cd-sync-metallb-crd.webp)
...
- #### Déploiment de Contour :
  ...
  **Chart utilisé** : [bitnami/contour](https://artifacthub.io/packages/helm/bitnami/contour) voir la [documentation officielle option 2](https://projectcontour.io/getting-started/). <br />
  ...
  - Exécuter la commande curl curl [http://remplacer-par-votre-domaine.com](http://remplacer-par-votre-domaine.com) ou sur votre navigateur web .
    ![Nginx Ingress Test](../assets/img/content/nginx-ingress-test.webp)
...
- [src/argocd/infra/overlays/lab/kustomization.yaml](https://raw.githubusercontent.com/mombe090/blog-source-code/refs/heads/argo-pihole-extd-certmanager/src/argocd/infra/overlays/lab/kustomization.yaml) :
  ...
  - Après la synchronisation vous devez voir les challenges de certificats :
    - Sur le dashboard d'argocd :
      ![Argocd Certifacates Challenges](../assets/img/content/argocd-certifacates-challenges.webp)
...
- Ajout de l'IP du serveur DNS **Pi-hole** à la configuration réseau de votre ordinateur pour le mettre comme l'un des DNS principale :
  ...
  ![Ajout Dns Config Machine](../assets/img/content/ajout-dns-config-machine.webp)
...
- [src/argocd/infra/base/common/argocd.httproute.yaml](https://raw.githubusercontent.com/mombe090/blog-source-code/refs/heads/argo-pihole-extd-certmanager/src/argocd/infra/base/common/argocd.httproute.yaml) :
  ...
  **Commit et push vos changements dans le dépôt git**
  ...
  ![Pihole Dns Added](../assets/img/content/pihole-dns-added.webp)
  ...
  - Maintenant si vous accédez à la [https://argocd.tuto.domain.com](https://argocd.tuto.domain.com) vous allez être redirigé sur une page https sécurisé :
    ![Argocd Https Login Page](../assets/img/content/argocd-https-login-page.webp)
  - On peut voir que let certificat est bien généré par lets encrypte et que la page est bien servie en **https**
    ![Argocd Letsencrypt](../assets/img/content/argocd-letsencrypt.webp)
...
- [src/argocd/infra/base/common/kustomization.yaml](https://raw.githubusercontent.com/mombe090/blog-source-code/refs/heads/argo-pihole-extd-certmanager/src/argocd/infra/base/common/kustomization.yaml) :
   ...
   ![Pi Hole Https](../assets/img/content/pi-hole-https.webp)
...
- Sur le dashboard d'argocd :
  - voici la nouvelle app qui gère les applications buisness :
    ![Buisness App Of Apps](../assets/img/content/buisness-app-of-apps.webp)
  - L'application métier s'appelle nginx et voici tous ces objets bien déployé et sync :
    ![Argocd Nginx App Synced](../assets/img/content/argocd-nginx-app-synced.webp)

## Acceder à l'application nginx sur le [https://myapp.tuto.votre-domaine.com](https://myapp.tuto.votre-domaine.com) :

![Nginx App Custom](../assets/img/content/nginx-app-custom.webp)
